...
def assign_cookie(greed: List[int], size: List[int]) -> int:
    result = i = 0
    greed.sort()
    size.sort()

    for g in greed:
        while i < len(size) and g > size[i]:
            i += 1
        if i == len(size):
            break
        result += 1
...
class Test_Assign_Cookie(unittest.TestCase):
    def test_returns1(self):
        test_greed = [1, 2, 3]
        test_size = [[1, 1], [3]]
        for s in test_size:
            assert assign_cookie(test_greed, s) == 1

    def test_returns2(self):
        test_greed = [10, 9, 8, 7]
        test_size = [5, 6, 7, 8]
        assert assign_cookie(test_greed, test_size) == 2
